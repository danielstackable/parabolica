...
interface StackProgram {
  ...
  Program_inLanguage: StackLanguage[];
  Program_occupationalCategory: StackOccupation[];
}
...
function renderProgramPage(program: StackProgram): string {
  ...
  const providerLinks = Array.isArray(provider.Provider_sameAs)
    ? provider.Provider_sameAs.map((link) => `<li><a href="${link}" target="_blank">${link}</a></li>`).join("")
    : "";

  const languages = program.Program_inLanguage.map((l) => l.Name).join(", ");
  const occupations = program.Program_occupationalCategory.map((o) => o.Name).join(", ");
...
function renderProviderPage(provider: StackProvider): string {
  const links = Array.isArray(provider.Provider_sameAs)
    ? provider.Provider_sameAs.map((link) => `<li><a href="${link}" target="_blank">${link}</a></li>`).join("")
    : "";
...
<!DOCTYPE html>
<html>
  <head><meta charset="utf-8" /><title>{provider.Provider_name}</title></head>
  <body>
...
(async () => {
  ...
  for (const domain of allowedDomains) {
    const programs = byDomain[domain] ?? [];
    const providers = providersByDomain[domain] ?? [];
    ...
    for (const provider of providers) {
      const relPath = path.join("projects", projectPath, "src/pages", provider.Provider_slug, "index.astro");
      writeFile(relPath, renderProviderPage(provider));
    }

    const indexPath = path.join("projects", projectPath, "src/pages/index.astro");
    writeFile(indexPath, renderIndexPage(programs, providers, domain));
  }
