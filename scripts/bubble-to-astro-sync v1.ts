/**********************************************************************
 *  bubble-to-astro-sync.ts — DEBUG VERSION
 *********************************************************************/
...
async function fetchAllPrograms(): Promise<StackProgram[]> {
  ...
  console.log(`📦 Fetched ${data.response.results.length} programs`);
...
async function fetchCanonicalDomain(id: string): Promise<string | null> {
  try {
    const data = await fetchJSON<{ response: StackCanonicalDomain }>(`${API_BASE}/Stack_CanonicalDomain/${id}`);
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Programs at ${domain}</title>
  </head>
  <body>
    <h1>Programs</h1>
    <ul>
...
(async () => {
  ...
  for (const program of programs) {
    ...
    for (const domain of matchedDomains) {
      ...
      console.log(`📝 Writing page to ${absPath}`);
      writeFile(absPath, renderProgramPage(program));
    }
  }
